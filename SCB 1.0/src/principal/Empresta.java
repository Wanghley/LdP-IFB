...
public class Empresta extends javax.swing.JFrame {
	public Empresta() throws IOException, InvalidFormatException {
		setTitle("SBC - Sistema de Controle Bibliotecário");
		initComponents();
		getTitles();
	}
	public void initComponents() throws IOException, InvalidFormatException {
		...
		JLabel lblDigiteOUsurio = new JLabel("Digite o usuário:");
	...
	private static void getTitles() throws IOException {
		jList1.setModel(new javax.swing.AbstractListModel<String>() { 
			String[] split = new Emprestimo().getBookList().split("#");
			public int getSize() { return split.length; }
			public String getElementAt(int i) { return split[i]; }
		});
	}
	private void ActionPerformed(ActionEvent e) throws RowsExceededException, WriteException, IOException {
		Emprestimo emprestimo = new Emprestimo();
		String user = txtUsuario.getText();
		boolean userExists = emprestimo.UserExists(user);
		...
		if(title==null) {
			...
			if(userExists) {
				boolean semDividas = emprestimo.UserCanBorrow(user);
				boolean emprestado = emprestimo.BookIsBorrowed(title);
				if(semDividas && emprestado) {
					...
					if(!semDividas) {
						JOptionPane.showMessageDialog(null,"Usuário com empréstimo pendente!",
								"Erro",JOptionPane.ERROR_MESSAGE);
					}else {
						JOptionPane.showMessageDialog(null,"Livro já foi emprestado!",
								"Erro",JOptionPane.ERROR_MESSAGE);
					}
				}
			}else {
		...
		java.awt.EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					Logger.getLogger(Empresta.class.getName()).log(Level.SEVERE, null, ex);
